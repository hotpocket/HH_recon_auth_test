...
class AuthService implements AuthServiceInterface {
  ...
  Future<bool> signIn() async {
    ...
    StreamSubscription<html.StorageEvent>? storageSubscription;
    ...
    messageSubscription = html.window.onMessage.listen((event) {
      if (event.data is Map) {
        ...
        if (data.containsKey('flutter-web-auth-2')) {
          if (!completer.isCompleted) {
            completer.complete(data['flutter-web-auth-2'] as String);
          }
          cleanup();
        }
      }
    ...
    try {
      channel = html.BroadcastChannel('flutter_web_auth_2');
      channelSubscription = channel.onMessage.listen((event) {
        if (event.data is Map) {
          final data = event.data as Map;
          if (data.containsKey('flutter-web-auth-2')) {
            if (!completer.isCompleted) {
      ...
      debugPrint('BroadcastChannel not supported: $e');
    }

    // Method 3: localStorage event
    storageSubscription = html.window.onStorage.listen((event) {
      if (event.key == 'flutter_web_auth_2_callback' &&
          event.newValue != null) {
        if (!completer.isCompleted) {
